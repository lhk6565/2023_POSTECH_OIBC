...
기존 논문을 기반으로 일사량과 같은 특성 추가, 날씨 실측값과 날씨 예측값을 활용한 날씨 예측값 보정 모델, 날씨 및 모델 예측값의 시계열성을 반영한 구조 등의 아이디어가 나왔으나, 도메인 지식 부족으로 최종 결과물에 반영되지 못한 부분은 아쉽다.<br>
또한, 입력에 따라 적응적으로 변화하는 가중치를 도출하였으나, 시간적인 제약으로 인해 이를 해석하여 새로운 인사이트를 도출하지 못한 점도 아쉽다.<br>
